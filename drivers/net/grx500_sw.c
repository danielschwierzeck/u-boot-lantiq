...
static _dma_rx_descriptor_t rx_des_ring[NUM_RX_DESC] __attribute__ ((aligned(32)));
...
static int ltq_grx500_switch_init(struct eth_device *dev, bd_t * bis)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		...
		rx_desc->DW2.all = ((CPHYSADDR((u32) net_rx_packets[i])) | 0x20000000) & 0xfffffff8;
		invalidate_dcache_range((u32)&rx_des_ring[i],(u32)(&rx_des_ring[i] + sizeof(_dma_rx_descriptor_t)));
	}
	...
	for (;;) {
		...
		if (length) {
			invalidate_scache_range((u32)net_rx_packets[rx_num],
					(u32)net_rx_packets[rx_num] + rx_desc->DW3.field.datalen);
			invalidate_dcache_range((u32)net_rx_packets[rx_num],
					(u32)net_rx_packets[rx_num] + rx_desc->DW3.field.datalen);
			net_process_received_packet((void*)KSEG1ADDR(net_rx_packets[rx_num]), length);
		} else {
...
int get_gphy_firmware(u32 dst, u32 src)
{
	...
	ulong unc_len = 0x20000; /* set uncompress len to 128k */
	u8* image_start = NULL;
	ulong image_len = 0;
	...
	while (1) {
		...
		case IH_TYPE_FIRMWARE:
			if (strncmp((const char *) pimg_header->ih_name,
				"GRX500 V1.1 GPHY BE", sizeof(pimg_header->ih_name)) == 0)
				fw_type = 1;
			...
			pimg_header = (image_header_t *)pimg_header
					+ sizeof(image_header_t)
					+ (pimg_header->ih_size);
			break;
		}
