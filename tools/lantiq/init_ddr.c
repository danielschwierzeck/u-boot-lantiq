...
static void mem_t(struct ddr_settings *ds);
static void dfi_t(struct ddr_settings *ds);
static void memmax_init(struct ddr_settings *ds);
static void fmt_init(void);
static void seed(u8 lane, u8 seedw, u8 seedr);

/************************************************************************
 *  Global variables
 ************************************************************************/

static int gate_ext = 1;
static FILE *fp;

/* Defaul Value Assigned for 512MB DDR3 9-9-9 667MHz single rank*/
static int ddr_type = 3;
static int ddr_grade = 6;
static int ddr_density = 8;
static int ddr_blen = 8;
static int ddr_rank = 1;
static int ddr_al = 1;
...
static int fgsw = 5;
static int fgswr = 6;
static int cgu_pll0a_cfg1 =
	0x40403866; /* cpu/ sl/ngi/cbm =  600/200/300/300 MHz */
static int cgu_pll0b_cfg1 =
	0x40401556; /* cpu/ddr/gsw/mii = 1000/250/330/250 MHz */
static int sr0[2] = { 24, 24 };
static int sr1[2] = { 24, 24 };
 ...
 *  Static variables
 ************************************************************************/
static const char *Progname = "init_ddr";
	...
	if ((ddr_type) == 2) {
		if ((ddr_grade) < 2) { //f=200MHz, tCK=5nS, 3-3-3
			...
			ds->xDDR_TXSNR =
				((ddr_density) < 2) ?
					...
					(28) :
					((ddr_density) < 8) ? (41) : (68);
		} else if ((ddr_grade) < 4) { //f=266MHz, tCK=3.75nS, 4-4-4
			ds->xDDR_TOGCNT1U =
				133; // 133*2*3.75=1uS (based on n_clk)
			...
			ds->xDDR_TRRD =
				((ddr_density) < 2) ?
					...
					((ddr_density) % 2 == 0) ?
					(3) :
					(2); //7.5-10nS, page always 1K for 256Mb
			ds->xDDR_TFAW_CFG = 5;
			ds->xDDR_TFAW_CFG_OFF =
				((ddr_density) < 2) ?
					(0) :
			...
			ds->xDDR_TXSNR =
				((ddr_density) < 2) ?
					...
					(37) :
					((ddr_density) < 8) ? (55) : (91);
		} else if ((ddr_grade) < 6) { //f=333MHz, tCK=3nS, 4-4-4/5-5-5
			...
			ds->xDDR_TRC = 19 + (((ddr_grade)) -
					     4); //min=57(4-4-4)/60nS(5-5-5)
			ds->xDDR_TFAW = ((ddr_density) < 2) ?
						(13) :
						((ddr_density) % 2 == 0) ?
						(17) :
			...
			ds->xDDR_TRRD =
				((ddr_density) < 2) ?
					...
					((ddr_density) % 2 == 0) ?
					(4) :
					(3); //7.5-10nS, page always 1K for 256Mb
			ds->xDDR_TFAW_CFG = 5;
			ds->xDDR_TFAW_CFG_OFF =
				((ddr_density) < 2) ?
					(2) :
			...
			ds->xDDR_TXSNR =
				((ddr_density) < 2) ?
					...
					((ddr_density) < 6) ?
					(47) :
					((ddr_density) < 8) ? (69) : (113);
		} else if ((ddr_grade) < 8) { //f=400MHz, tCK=2.5nS, 5-5-5/6-6-6
			ds->xDDR_TOGCNT1U =
				200; // 200 *2*2.5=1uS (based on n_clk)
			ds->xDDR_TOGCNT100N = 20; //  20 *2*2.5=100nS
			ds->xDDR_TREFPRD = 28080; //9*7800/2.5=28080
			ds->xDDR_TRAS = 18; //min=45nS
			ds->xDDR_TRC = 23 + (((ddr_grade)) -
					     6); //min=57.5(5-5-5)/60nS(6-6-6)
			ds->xDDR_TFAW = ((ddr_density) < 2) ?
						(14) :
						((ddr_density) % 2 == 0) ?
						(18) :
			...
			ds->xDDR_TXSNR =
				((ddr_density) < 2) ?
					...
					((ddr_density) < 6) ?
					(55) :
					((ddr_density) < 8) ? (82) : (135);
		} else {
			//f=533MHz, tCK=1.87nS, 7-7-7
			ds->xDDR_TOGCNT1U =
				267; // 268*2*1.876=1uS (based on n_clk)
			...
			ds->xDDR_TRRD =
				((ddr_density) < 2) ?
					...
					((ddr_density) % 2 == 0) ?
					(6) :
					(4); //7.5-10nS, page always 1K for 256Mb
			ds->xDDR_TFAW_CFG =
				((ddr_density) < 2) ?
					(5) :
					((ddr_density) % 2 == 0) ? (4) : (5);
		...
		ds->xDDR_TCL =
			((ddr_grade) < 2) ?
				...
				((ddr_grade) < 6) ?
				((ddr_grade)) :
				((ddr_grade) < 8) ? (((ddr_grade)) - (1)) : (7);
		ds->xDDR_TRP = ((ds->xDDR_BANKSZ) == 8) ?
				       ((1 << 16) + (ds->xDDR_TCL)) :
				       (ds->xDDR_TCL);

		ds->xDDR_TCWL = ((ds->xDDR_TCL) - (1));

		ds->xDDR_DFITPHYWRLAT =
			(((((ds->xDDR_TCWL) + (ds->xDDR_TAL)) - (1)) >> 1) -
			 (1));
		ds->xDDR_DFITRDDATAEN =
			(((((ds->xDDR_TCWL) + (ds->xDDR_TAL))) >> 1) - (1));

		ds->xDDR_TRCD = (ds->xDDR_TCL);
		ds->xDDR_TRTP = ((ddr_grade) < 2) ?
					(2) :
					((ddr_grade) < 4) ?
					(2) :
					((ddr_grade) < 6) ?
		...
		ds->xDDR_MR0 = (((ddr_blen) == 8) ? (3) : (2)) +
			       ((ds->xDDR_TCL) << 4) +
			       (((ds->xDDR_TWR) - 1) << 9) + (0x100);

		if ((ddr_grade) < 6)
			ds->xDDR_MR1 = (((ddr_al) << 3) +
					(0x04)); //  75Ohm, full drive
		else
			ds->xDDR_MR1 = (((ddr_al) << 3) +
					(0x40)); // 150Ohm, full drive
		...
		ds->xMM_DRAMCONFIG = (0x100c0020) +
				     (((ddr_blen) == 8) ? (0x100) : (0)) +
				     (((ds->xDDR_BANKSZ) == 8) ? (1) : (0));

		if (ddr_rank == 1) {
			ds->xMM_SWAP0_H = ((ddr_density) < 1) ?
						  (0x0011634c) :
						  ((ddr_density) < 3) ?
						  (0x00000117) :
						  ((ddr_density) < 4) ?
		...
		if ((ddr_grade) < 2) { //f=400MHz, tCK=2.5nS, 5-5-5/6-6-6
			...
			ds->xDDR_TXS =
				((ddr_density) < 4) ?
					...
					((ddr_density) < 8) ?
					(68) :
					((ddr_density) < 10) ? (108) : (144);
		} else if ((ddr_grade) <
			   5) { //f=533MHz, tCK=1.87nS, 7-7-7/8-8-8
			ds->xDDR_TOGCNT1U =
				267; // 268*2*1.876=1uS		(based on n_clk)
			ds->xDDR_TOGCNT100N = 27; //  27*2*1.876=100nS
			...
			ds->xDDR_TRRD =
				((ddr_density) > 9) ?
					...
					((ddr_density) % 2 == 0) ?
					(6) :
					(4); //7.5-10nS, page always 2K for 8Gb
			ds->xDDR_TFAW_CFG = 5;
			ds->xDDR_TFAW_CFG_OFF =
				((ddr_density) > 9) ?
					(3) :
			...
			ds->xDDR_TXS =
				((ddr_density) < 4) ?
					...
					((ddr_density) < 8) ?
					(72) :
					((ddr_density) < 10) ? (145) : (193);
		} else if ((ddr_grade) <
			   8) { //f=667MHz, tCK=1.5nS, 9-9-9/10-10-10
			ds->xDDR_TOGCNT1U =
				333; // 333*2*1.5=1uS 		(based on n_clk)
			...
			ds->xDDR_TRRD =
				((ddr_density) > 9) ?
					...
					((ddr_density) % 2 == 0) ?
					(5) :
					(4); //6-7.5nS, page always 2K for 8Gb
			ds->xDDR_TFAW_CFG =
				((ddr_density) > 9) ?
					(6) :
					((ddr_density) % 2 == 0) ? (6) : (5);
			...
			ds->xDDR_TXS =
				((ddr_density) < 4) ?
					...
					((ddr_density) < 8) ?
					(113) :
					((ddr_density) < 10) ? (180) : (240);
		} else {
			//f=800MHz, tCK=1.25nS, 10-10-10/11-11-11
			ds->xDDR_TOGCNT1U =
				400; //400  *2*1.25=1uS 		(based on n_clk)
			...
			ds->xDDR_TRRD =
				((ddr_density) > 9) ?
					...
					((ddr_density) % 2 == 0) ?
					(6) :
					(5); //6-7.5nS, page always 2K for 8Gb
			ds->xDDR_TFAW_CFG =
				((ddr_density) > 9) ?
					(6) :
					((ddr_density) % 2 == 0) ? (6) : (5);
		...
		ds->xDDR_TRSTL =
			((ddr_grade) < 2) ?
				...
				((ddr_grade) < 5) ?
				(55) :
				((ddr_grade) < 8) ? (68) : (82); //100nS + 2CK
		ds->xDDR_TCL = ((ddr_grade) < 2) ?
				       (((ddr_grade)) + 5) :
				       ((ddr_grade) < 5) ?
				       (((ddr_grade)) + 4) :
				       ((ddr_grade) < 8) ? (((ddr_grade)) + 3) :
							   (((ddr_grade)) + 1);
		ds->xDDR_TAL =
			((ddr_al) > 0) ? ((ds->xDDR_TCL) - (ddr_al)) : (0);
		ds->xDDR_TRP = (ds->xDDR_TCL);

		ds->xDDR_TCWL = ((ddr_grade) < 2) ?
					(5) :
					((ddr_grade) < 5) ?
					(6) :
					((ddr_grade) < 8) ? (7) : (8);
		...
		ds->xDDR_MR0 =
			...
			(((ds->xDDR_TCL) <= 11) ?
				 ...
				 ((((ds->xDDR_TCL) - 11) << 4) + (4))) +
			((1) << 8) +
			(((ds->xDDR_TWR) <= 8) ? (((ds->xDDR_TWR) - 4) << 9) :
						 (((ds->xDDR_TWR) >> 1) << 9));

		if ((ddr_grade) < 8) {
			ds->xDDR_MR1 = (((ddr_al) << 3) +
					(0x46)); //ODS=34 Ohm, Rtt=34hm (667MHz)
			ds->xDDR_MR2 = ((((ds->xDDR_TCWL) - 5) << 3) +
					(0x200)); //Dyn ODT Rtt = 60 Ohm
		} else {
			ds->xDDR_MR1 =
				(((ddr_al) << 3) +
				 (0x44)); //ODS=40 Ohm, Rtt=40 Ohm (800MHz)
			ds->xDDR_MR2 = ((((ds->xDDR_TCWL) - 5) << 3) +
					(0x400)); //Dyn ODT Rtt = 120 Ohm
		}
		...
		ds->xMM_DRAMCONFIG = (0x100c0030) +
				     (((ddr_blen) == 8) ? (0x100) : (0)) +
				     (((ds->xDDR_BANKSZ) == 8) ? (1) : (0));

		if ((ddr_rank) == 1) {
			ds->xMM_SWAP0_H = 0;
			ds->xMM_SWAP0_L = ((ddr_density) < 3) ?
						  (0x0559028b) :
						  ((ddr_density) < 5) ?
						  (0x0461128b) :
						  ((ddr_density) < 7) ?
...
static void seed(u8 lane, u8 seedw, u8 seedr)
{
	u32 xw, xr, x2;
	u32 cmd;
	xw = (seedw << 24) + (seedw << 18) + (seedw << 12) + (seedw << 6) +
	     seedw;
	x2 = (seedr << 18) + (seedr << 12) + (seedw << 6) + seedw;
	xr = (seedr << 24) + (seedr << 18) + (seedr << 12) + (seedr << 6) +
	     seedr;
...
{
	u32 cmd;
	cmd = (0x80000000 + (((ds->xDDR_TREFI_N) - 1) << 16) +
	       (ds->xDDR_TREFI_C));
...
static void dfi_t(struct ddr_settings *ds)
{
	...
	cmd = (ddr_rank == 1) ?
		      0x08 :
		      0x0808; //Enable ODT for rank 0 and 1 when a write access on this rank
	__prt_wr("DFIODTCFG", DFIODTCFG_REG_addr, cmd);

	if (ddr_type == 2) {
		if ((ddr_grade) < 9) //All DDR2 except DDR2-1066
			cmd = 0x06060000 +
			      (((ds->xDDR_TCL) + (ds->xDDR_TAL) - 3) << 8) +
			      ((ds->xDDR_TCWL) + (ds->xDDR_TAL) - 3);
		else
			cmd = 0x07070000 +
			      (((ds->xDDR_TCL) + (ds->xDDR_TAL) - 4) << 8) +
			      ((ds->xDDR_TCWL) + (ds->xDDR_TAL) - 4);
	} else {
	...
	cmd = (ddr_rank == 1) ?
		      0x01 :
		      0x21; //dfi_odt[x] will be asserted to terminate rank x (x=1,2)
...
static void parse_args1(int argc, char *argv[])
{
	int i;
	u32 x;
	char *str;
	...
	printf("\tType:%d Grade:%d Density:%d BL:%d AL:%d Rank:%d Endian:%d SR0:%d %d SR1:%d %d\n",
	       ddr_type, ddr_grade, ddr_density, ddr_blen, ddr_al, ddr_rank,
	       endianness, sr0[0], sr0[1], sr1[0], sr1[1]);

	if (fcps < 16) {
		cgu_pll0a_cfg1 = 0x40400000 + (fcps << 12) + (fsl << 8) +
				 (fngi << 4) + (fcbm);
	} else {
		cgu_pll0a_cfg1 = 0x60400000 + (3 << 12) + (fsl << 8) +
				 (fngi << 4) + (fcbm);
	}
	cgu_pll0b_cfg1 = 0x40400000 + (fcps2 << 12) + (fdctl << 8) +
			 (fgsw << 4) + (fgswr);
...
int main(int argc, char *argv[])
{
	...
	// s2
	cmd = ((((ds.xDDR_TFAW_CFG) - 4) << 18) + (((ddr_type)-2) << 5) +
	       ((ddr_blen == 8) ? (1) : (0)));
	...
	cmd = ((ds.xDDR_TRC) << 26) + ((ds.xDDR_TRRD) << 22) +
	      ((ds.xDDR_TRAS) << 16) + ((ds.xDDR_TRCD) << 12) +
	      ((ds.xDDR_TCL) << 8) + ((ds.xDDR_TWTR) << 4) + (ds.xDDR_TRTP);
	//TRP always = TCL
	__prt_wr("PUB_DTPR0", PUB_DTPR0_REG_addr, cmd);
	cmd = ((ds.xDDR_TAOND) << 30) +
	      ((((ddr_type) == 2) ? (8) : (ds.xDDR_TWLO)) << 26) +
	      ((((ddr_type) == 2) ? (40) : (ds.xDDR_TWLMRD)) << 20) +
	      ((ds.xDDR_TRFC) << 11) + ((ds.xDDR_TFAW) << 5) +
	      ((((ddr_type) == 2) ? (4) : ((ds.xDDR_TMOD) - 12)) << 2) +
	      (((ddr_type) == 2) ? (ds.xDDR_TMRD) : ((ds.xDDR_TMRD) - 4));
	//Some unused in DDR2, keep default
	__prt_wr("PUB_DTPR1", PUB_DTPR1_REG_addr, cmd);

	cmd = (((ds.xDDR_TCCD) > 4) ? (1 << 31) : (0)) +
	      ((ds.xDDR_TDLLK) << 19) + ((ds.xDDR_TCKE) << 15) +
	      ((ds.xDDR_TXP) << 10) + (ds.xDDR_TXS) + (1 << 30);
	__prt_wr("PUB_DTPR2", PUB_DTPR2_REG_addr, cmd);

	cmd = (((ddr_type) == 2) ? (0x00f00000) : (0x00f80000)) +
	      (ds.xDDR_TREFPRD);
	//DDR3 uses Fixed Latency
	__prt_wr("PUB_PGCR2", PUB_PGCR2_REG_addr, cmd);

	cmd = (ds.xDDR_TPHYRST) + ((ds.xDDR_TPLLGS) << 6) +
	      ((ds.xDDR_TPLLPD) << 21);
	__prt_wr("PUB_PTR0", PUB_PTR0_REG_addr, cmd);
	cmd = (ds.xDDR_TPLLRST) + ((ds.xDDR_TPLLLOCK) << 16);
	__prt_wr("PUB_PTR1", PUB_PTR1_REG_addr, cmd);
	cmd = (ds.xDDR_TCALON) + ((ds.xDDR_TCALS) << 5) +
	      ((ds.xDDR_TCALH) << 10) + ((ds.xDDR_TWLDLYS) << 15);
	...
	if ((ddr_type) == 2) {
		...
		cmd = (0x80f00003 |
		       (((ds.xDDR_MR0) & (~0x100))
			<< 4)); //PLL reset=0, for timing programming
		__prt_wr("MCMD", MCMD_REG_addr, cmd);
		read_reg = cmd & 0x7fffffff;
		__prt_rd("MCMD", MCMD_REG_addr, read_reg);
		__delay();
	...
	*******************************************************/
	...
	for (rank = 0; rank < ddr_rank; rank++) {
		if (ddr_type == 2)
			cmd = 0x90007887 | (((ddr_rank) == 2 ? 3 : 1) << 24) |
			      (rank << 4);
		else {
			if ((ddr_grade) > 5)
				cmd = 0x900035c7 | ((rank + 1) << 24) |
				      (rank << 4);
			//def=0x93003587 high freq uses 1/4CK 0x910035c7
			else
				cmd = 0x900078c7 | ((rank + 1) << 24) |
				      (rank << 4);
