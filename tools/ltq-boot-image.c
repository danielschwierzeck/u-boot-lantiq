...
{
	fprintf(stderr, "%s: [-h] -t type -e entry-addr -x uboot-offset [-X tpl-offset] -U uboot-size [-p page-size] -u uboot-bin [-s spl-bin] [-T spl-bin] -o out-bin\n",
		name);
...
static int parse_args(int argc, char *argv[], struct args *arg)
{
	...
	while ((opt = getopt(argc, argv, "ht:e:x:X:U:p:u:s:T:o:")) != -1) {
		switch (opt) {
		case 'h':
			usage_msg(argv[0]);
			return 1;
