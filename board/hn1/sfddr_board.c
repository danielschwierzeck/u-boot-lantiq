...
void uncompress_image(ulong image_addr)
{
   ...
   //mem_malloc_init(0x82000000, TOTAL_MALLOC_LEN);   //AT: This assumes 32MBytes of external RAM!
   mem_malloc_init((CONFIG_IFX_MEMORY_SIZE*1024*1024)+0x80000000, TOTAL_MALLOC_LEN);
   addr = image_addr;
...
//AT: FIXME: for STMicro chip (that is used by Palladium) we reuse the same values as for MXIC. To be confirmed that are OK for all the chips
//AT: CMD_MX25L_READ = 0x03. HN1 bootROM uses  EEPROM_READ command code = 0x03.
//AT: a length of 3 corresponds to 3 blocks of 8 bits of address => 24bits =>16MBytes maximum size.
    u8  cmd = CMD_MX25L_READ;
    size_t  addr_len = 3;
//AT: Don't forget to also update sfpreload.c file!
//#endif
#endif	
    ...
    asm("sync");
    mem_malloc_init (0xbf290000, 200);
    asm("sync");
    ...
    //asc_puts("\ncall tune_ddr()\n");
#ifndef CONFIG_FASTER_UART
    tune_ddr();
#else
    asc_puts("FIXME: skipped DDR tuning!\n");
#endif
    //asc_puts("returned from tune_ddr()\n");

#ifdef CONFIG_BOOT_FROM_SPI
	#ifdef CONFIG_DEBUG
    asc_puts("copy compressed U-Boot: 64KB from 0x5000 to 0x"); print_u32(CONFIG_BOOTSTRAP_TEXT_BASE); asc_puts("\n");
	#endif
	sf_read(cmd,(ulong)0x5000,addr_len,(u8*)CONFIG_BOOTSTRAP_TEXT_BASE,0x10000);  /*copy u-boot image*/
	
    #ifdef CONFIG_HN1_GPHY_FW_ADDR
        //AT: FIXME If the image has 192kB total then gphy image is stored at 0x2.0000
        //AT: The code from below assumes an 128kB image.
        //AT: Modified to copy to A011.0000 (=same as CONFIG_HN1_GPHY_FW_ADDR)
	    #ifdef CONFIG_DEBUG
        asc_puts("copy gphy: 64KB from 0x1.0000 to 0x"); print_u32(CONFIG_HN1_GPHY_FW_ADDR); asc_puts("\n");
	    #endif
        sf_read(cmd,(ulong)0x10000,addr_len,(u8*)CONFIG_HN1_GPHY_FW_ADDR,0x10000);  /*copy gphy firmware*/
    #endif
   ...
   #ifdef CONFIG_DEBUG
   asc_puts("uncompress image\n");
   #endif
   uncompress_image(CONFIG_BOOTSTRAP_TEXT_BASE);
