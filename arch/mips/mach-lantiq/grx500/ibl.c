...
#define CONFIG_TEXT_BASE_ADDR CONFIG_SPL_TEXT_BASE
...
static u32 tc_getinfo(int8_t cpu, TC_launch_t tc_launch[])
{
	...
	for (i = 0; i < MAX_TCS; i++) {
		if ((tc_launch[i].tc_num > 0) &&
		    (tc_launch[i].tc_num < MAX_TC)) {
			tcmask = tcmask | (0x1 << tc_launch[i].tc_num);
			set_tc_policy(tc_launch[i].tc_num,
				      tc_launch[i].mt_group);
		}
	}
...
void vpe_launch(int cpu, int c_id, int vpe_id, CPU_launch_t cpu_launch,
		TC_launch_t tc_launch[])
{
	...
	__write_32bit_c0_register($11, 0,
				  (__read_32bit_c0_register($9, 0) + 1000));
	asm volatile("ehb");
...
void ibl(u32 cpu_num)
{
	...
	while (1) {
		...
		if (l_vmb_t.msg_id == VMB_CPU_START) {
			...
			vpe_launch(cpu_num, c_id, v_id, l_vmb_t.cpu_launch,
				   l_vmb_t.tc_launch);
		}
	}
}
